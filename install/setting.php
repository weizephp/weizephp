...
if(!isset($_POST['dbhost'])) {
    ...
    $sql = "INSERT INTO `{$tablepre}user` (`email`, `username`, `mobile`, `password`, `status`, `roleid`, `regtime`, `regip`, `lastlogintime`, `lastloginip`, `logincount`, `point`, `balance`, `salt`) VALUES ('{$email}', '{$username}', '0', '{$password}', '1', '1', '".INSTALL_TIMESTAMP."', '{$_SERVER['REMOTE_ADDR']}', '0', '', '0', '0', '0.00', '{$salt}')";
    $DB->query($sql);
    ...
    // 配置文件变量
    $cfg_filename = '../config/w_config.inc.php';
    ...
    // 替换站点安全密钥
    $cfg_content = str_replace("\$wconfig['authkey']         = 'cblL5knE4k8ET8g1';", "\$wconfig['authkey']         = '{$random_str}';", $cfg_content);
    
    // 数据库主机
    $cfg_content = str_replace("\$wconfig['db']['host']     = 'localhost';", "\$wconfig['db']['host']     = '{$dbhost}';", $cfg_content);
    // 数据库用户名
    $cfg_content = str_replace("\$wconfig['db']['username'] = 'root';",      "\$wconfig['db']['username'] = '{$dbuser}';", $cfg_content);
    // 数据库密码
    $cfg_content = str_replace("\$wconfig['db']['password'] = '';",          "\$wconfig['db']['password'] = '{$dbpw}';", $cfg_content);
    // 数据库名
    $cfg_content = str_replace("\$wconfig['db']['name']     = 'weizephp4';", "\$wconfig['db']['name']     = '{$dbname}';", $cfg_content);
    // 数据库端口（默认是3306）
