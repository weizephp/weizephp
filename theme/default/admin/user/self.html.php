...
<html>
    <head>
        <?php include $wconfig['theme_path'] . '/admin/head.inc.php';echo admin_head('个人信息修改');?>
    </head>
    <body>
        <?php include $wconfig['theme_path'] . '/admin/header.html.php';?>
            ...
            <form class="form-horizontal w-form" autocomplete="off">
                ...
                <div class="form-group">
                    <label for="password_again" class="col-sm-2 control-label">新密码确认</label>
                    <div class="col-sm-10">
        ...
        $(document).ready(function() {
            $('.w-form').submit(function() {
                $.post("?m=user&a=self", $(this).serialize(), function(result) {
                    if(result.status == 1) {
                        if(result.logout == 'yes') {
                            w_dialog_success(result.msg, '?m=login');
                        } else {
