...
?>              
    ...
    <form class="w-form">
        ...
        <div class="form-group">
            <label for="categoryid">分类<span class="text-danger">(必填)</span></label>
        ...
        <div class="form-group">
            <label for="sourceurl">文章来源网址</label>
    ...
    $(document).ready(function() {
    	...
    	$('.w_upload_pic').click(function(e) {
        	$.ajax_file_upload(
        		{url: 'generals.php?m=upload&a=upload&folder=article'},
        		function(data) {
            		if(data) {
                    ...
                    if(result.status == 1) {
                        var thumbHtml = '<div class="thumbnail">' +
                            '<img src="generals.php?m=image&a=thumb&f=article&s='+ result.data.save_name +'&w=242&h=200"/>' +
        ...
        $('.w-form').submit(function() {
            ...
            $.post("?m=article&a=add", formData, function(result) {
                if(result.status == 1) {
                	w_b_modal_success('操作成功', function() {
                    	window.location.href = '?m=article&a=list';
                    });
                } else {
                	w_b_modal_error(result.msg);
                }
