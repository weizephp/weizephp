...
if(!isset($_POST['title'])) {
    ...
    // 获取文章
    $sql = "SELECT * FROM `{$_W['db']['tablepre']}articles` WHERE `aid`='{$aid}'";
    $row = $WDB->get_row($sql);
    ...
    // 获取内容
    $sql     = "SELECT * FROM `{$_W['db']['tablepre']}articlescontents` WHERE `aid`='{$aid}'";
    ...
    // 载入模板
    include $_W['template_path'].'/article/update.html.php';
    ...
    // 4、读取旧数据
    $sql = "SELECT `aid`, `categoryid`, `status`, `title`, `pic` FROM `{$_W['db']['tablepre']}articles` WHERE `aid`='{$aid}'";
    $old = $WDB->get_row($sql);
    if(empty($old)) {
    ...
    // 11、把数据更新进数据库
    $sql = "UPDATE `{$_W['db']['tablepre']}articles` SET `categoryid`='{$categoryid}',`status`='{$status}',`displayorder`='{$displayorder}',`title`='{$title}',`subtitle`='{$subtitle}',`keywords`='{$keywords}',`description`='{$description}',`pic`='{$pic}',`updatetime`='".W_TIMESTAMP."',`jumpurl`='{$jumpurl}',`source`='{$source}',`sourceurl`='{$sourceurl}',`editor`='{$_W['user']['username']}' WHERE `aid`='{$aid}'";
    if($WDB->query($sql)) {
        // 更新内容
        $sql = "UPDATE `{$_W['db']['tablepre']}articlescontents` SET `content`='{$content}' WHERE `aid`='{$aid}'";
        $WDB->query($sql);
        ...
        if(!empty($attachments)) {
            foreach($attachments as $v) {
                ...
                if(is_file($tmp_filename)) {
                    ...
                    $sql = "INSERT INTO `{$_W['db']['tablepre']}articlesattachments`(`aid`, `uid`, `attachment`) VALUES ('{$aid}', '{$_W['user']['uid']}', '{$v}')";
                    $WDB->query($sql);
                }
            }
        }
        ...
        foreach($old['attachments'] as $v) {
            if(!in_array($v['attachment'], $old_attachments)) {
                ...
                if(is_file($attachment_filename)) {
                    $sql = "DELETE FROM `{$_W['db']['tablepre']}articlesattachments` WHERE `id`='{$v['id']}'";
                    $WDB->query($sql);
        ...
        // 增加管理记录
        if($old['title'] != $title) {
            w_adminlog('把文章“'. $old['title'] .'”更新为：'. $title);
        } else {
            w_adminlog("更新了文章：". $title);
        }
