...
$upload = new w_upload();
$upload->max_size = 2;
$upload->allow_extension('gif', 'jpg', 'jpeg', 'png', 'bmp');
$upload->save_path = './upload';
$result = $upload->upload($_FILES['file']);
switch($result) {
    case 1:
        exit('{"status":0, "msg":"上传的文件大小超过了系统配置的限制"}');
        break;
    case 2:
        exit('{"status":0, "msg":"上传文件的大小超过了表单规定的最大值"}');
        break;
    case 3:
        exit('{"status":0, "msg":"文件只有部分被上传"}');
        break;
    case 4:
        exit('{"status":0, "msg":"没有文件被上传"}');
        break;
    case 6:
        exit('{"status":0, "msg":"找不到临时文件夹"}');
        break;
    case 7:
        exit('{"status":0, "msg":"文件写入失败"}');
        break;
    case 8:
        exit('{"status":0, "msg":"上传文件大小不符"}');
        break;
    case 9:
        exit('{"status":0, "msg":"上传文件MIME类型不允许"}');
        break;
    case 10:
        exit('{"status":0, "msg":"上传的文件后缀类型不允许"}');
        break;
    case 11:
        exit('{"status":0, "msg":"非法上传文件"}');
        break;
    case 12:
        exit('{"status":0, "msg":"非法图像文件"}');
        break;
    case 13:
        exit('{"status":0, "msg":"文件夹不可写"}');
        break;
    case 14:
        exit('{"status":0, "msg":"同名文件已经存在"}');
        break;
    case 15:
        exit('{"status":0, "msg":"保存文件夹创建失败"}');
        break;
    case 16:
        exit('{"status":0, "msg":"将上传的文件移动到新位置出错"}');
        break;
}
// 最后，把上传文件信息($result)保存进数据库...
*/
